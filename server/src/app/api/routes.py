"""
API routes - Legacy endpoint for send-book
All other routes have been moved to modular routers:
- /api/register, /api/login -> routers/auth.py
- /api/documents/* -> routers/documents.py
- /api/moderation/* -> routers/moderation.py
"""
...
async def send_book(
    ...
    try:
        ...
        print(f"❌ Error in send_book: {e}")
        print(f"Full traceback:\n{error_details}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed OCR processing or document creation: {str(e)}"
        )
